"""Smoke test: assemble a hand-built tree, compile to MJCF, load in MuJoCo.

Run from the project root:  python -m tests.test_pipeline
"""
...
def build_example_tree() -> RobotTree:
...
def main():
    ...
    xml = builder.to_xml_string(tree)

    model = mujoco.MjModel.from_xml_string(xml)
    data = mujoco.MjData(model)
    mujoco.mj_step(model, data)  # advance one step to confirm it simulates

    print("MJCF compiled and stepped OK")
